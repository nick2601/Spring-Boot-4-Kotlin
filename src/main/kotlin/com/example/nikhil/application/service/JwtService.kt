...
import com.example.nikhil.infrastructure.security.JwtConfig
...
class JwtService(
    ...
    fun getEmailFromToken(token: String): String? = try { getClaims(token)?.subject } catch (e: Exception) { logger.debug("Failed to extract email from token: ", e); null }
    fun getUserIdFromToken(token: String): Long? = try { getClaims(token)?.get("userId", Number::class.java)?.toLong() } catch (e: Exception) { logger.debug("Failed to extract userId from token: ", e); null }
    fun getNameFromToken(token: String): String? = try { getClaims(token)?.get("name", String::class.java) } catch (e: Exception) { logger.debug("Failed to extract name from token: ", e); null }
    @Suppress("UNCHECKED_CAST")
    fun getRolesFromToken(token: String): List<String> = try { getClaims(token)?.get("roles", List::class.java) as? List<String> ?: emptyList() } catch (e: Exception) { logger.debug("Failed to extract roles from token: ", e); emptyList() }
    fun getTokenIdFromToken(token: String): String? = try { getClaims(token)?.id } catch (e: Exception) { logger.debug("Failed to extract token ID from token: ", e); null }
    fun getIssuerFromToken(token: String): String? = try { getClaims(token)?.issuer } catch (e: Exception) { logger.debug("Failed to extract issuer from token: ", e); null }
    fun getIssuedAtFromToken(token: String): Date? = try { getClaims(token)?.issuedAt } catch (e: Exception) { logger.debug("Failed to extract issued at time from token: ", e); null }
    fun getExpirationFromToken(token: String): Date? = try { getClaims(token)?.expiration } catch (e: Exception) { logger.debug("Failed to extract expiration time from token: ", e); null }
    fun getAllClaimsFromToken(token: String): Map<String, Any>? = try { getClaims(token) } catch (e: Exception) { logger.debug("Failed to extract all claims from token: ", e); null }
    fun getTokenType(token: String): String? = try { getClaims(token)?.get("type", String::class.java) } catch (e: Exception) { logger.debug("Failed to extract token type: ", e); null }
    ...
    fun validateToken(token: String, email: String): Boolean = try {
        ...
        isValid
    } catch (e: Exception) { logger.debug("Token validation error: ", e); false }
    fun validateAccessToken(token: String, email: String): Boolean = validateToken(token, email) && isAccessToken(token)
    fun validateRefreshToken(token: String, email: String): Boolean = validateToken(token, email) && isRefreshToken(token)
    fun isTokenExpired(token: String): Boolean = try { val expiration = getExpirationFromToken(token); expiration?.before(Date()) ?: true } catch (e: Exception) { logger.debug("Error checking token expiration: ", e); true }
    fun needsRefresh(token: String): Boolean {
        ...
        } catch (e: Exception) { logger.debug("Error checking if token needs refresh: ", e); false }
    }
    ...
    } catch (e: Exception) { logger.debug("Error getting time until expiry: ", e); 0 }
    ...
    private fun getClaims(token: String): Claims? = try {
        Jwts.parser()
            ...
            .payload
    } catch (e: Exception) { logger.debug("Failed to parse token: ", e); null }
}
